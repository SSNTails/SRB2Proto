...
// Blue Shield obtained Tails 12-05-99
...
//    P_SpawnMobj (plyr->mo->x, plyr->mo->y, plyr->mo->z + (plyr->mo->info->height / 2), MT_BLUEORB);
    S_StartSound (plyr->mo, sfx_getpow);
     }
}
// end Blue Shield code Tails 12-05-99

// Start Ringbox codes Tails

void A_RingBox (mobj_t* source)
{
 if (!multiplayer)
   {
...
void A_Invincibility ()
{
 if (!multiplayer)
    {
       plyr->powers[pw_invulnerability] = 896;
       P_SpawnMobj (plyr->mo->x, plyr->mo->y, plyr->mo->z + (plyr->mo->info->height /2), MT_IVSR);
       S_StopMusic();
...
void A_Ring_Chase (mobj_t*   actor)
{
    int         delta;
